 #include <bits/stdc++.h>

using namespace std;

 

//Hàm tìm ki?m nh? phân

int binarySearch(int arr[], int left, int right, int x) {

    int middle;

 

    while(left <= right) {

        // L?y v? trí ? gi?a left và right

        middle = (left + right) / 2;

 

        // N?u ph?n t? ? gi?a b?ng x thì tr? v?

        // v? trí

        if (arr[middle] == x)

            return middle;

 

        // N?u x l?n hon ph?n t? ? gi?a thì

        // ch?c ch?n nó n?m bên ph?i.

        // ? dây chúng ta ch? d?nh left là ph?n t? ? gi?a + 1

        if (x > arr[middle])
            left = middle + 1;
        else
            //Ngu?c l?i
            right = middle – 1;

    }

 

    //Tr? v? -1 n?u nhu không tìm du?c giá tr? dó trong m?ng.

    return -1;

}

int main() {

	int arr[] = {15, 20, 25, 30, 31, 44, 66};

 

    //L?y ra d? dài c?a m?ng

    int n = sizeof(arr) / sizeof(arr[0]);

    //Ph?n t? c?n tìm

    int x = 25;

     

    // n -1 du?c xác d?nh là v? trí cu?i cùng trong m?ng

    int result = binarySearch hay có (arr, 0, n-1, x);

 

    cout << result;

}